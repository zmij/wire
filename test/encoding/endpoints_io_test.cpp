...
TEST(Interface, Resolve)
{
    using detail::inet_interface;
    {
        auto loopbacks =
            util::get_local_interfaces(util::get_interface_options::loopback |
                    util::get_interface_options::ip4);
        ASSERT_FALSE(loopbacks.empty()) << "Obtained a list of loopback interfaces";
        inet_interface iface{ loopbacks.front(), inet_interface::version_type::v4 };
        auto host = iface.resolve_address();
        EXPECT_FALSE(host.empty()) << "Resolved IP address for a loopback interface";
    }
    {
        auto loopbacks =
            util::get_local_interfaces(util::get_interface_options::loopback |
                    util::get_interface_options::ip6);
        ASSERT_FALSE(loopbacks.empty()) << "Obtained a list of loopback interfaces";
        inet_interface iface{ loopbacks.front(), inet_interface::version_type::v6 };
        auto host = iface.resolve_address();
        EXPECT_FALSE(host.empty()) << "Resolved IP address for a loopback interface";
    }
}
