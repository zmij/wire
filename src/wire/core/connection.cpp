        ...
        ::std::exception_ptr ex)
{
    ...
    if (pending_replies_.find(acc, r_no)) {
        #if DEBUG_OUTPUT >= 3
        ::std::ostringstream os;
        tag(os) << " Request " << r_no << " connection error\n";
        ::std::cerr << os.str();
        #endif
        ...
        functional::void_callback cb)
{
    #if DEBUG_OUTPUT >= 3
    ::std::ostringstream os;
    tag(os) << " Send " << out->type() << " size " << out->size() << "\n";
    ::std::cerr << os.str();
    #endif
        ...
        functional::void_callback cb, encoding::outgoing_ptr out)
{
    if (!ec) {
        ...
        #if DEBUG_OUTPUT >= 2
        ::std::ostringstream os;
        tag(os) << " Write failed " << ec.message() << "\n";
        ::std::cerr << os.str();
        #endif
        connection_failure(
    ...
    switch(m.type()) {
        ...
        default: {
            ...
            if (!incoming->complete()) {
                #if DEBUG_OUTPUT >= 3
                ::std::ostringstream os;
                tag(os) << " Wait for more data from peer\n";
                ::std::cerr << os.str();
                #endif
                incoming_ = incoming;
            } else {
...
connection_implementation::read_incoming_message(incoming_buffer_ptr buffer, ::std::size_t bytes)
{
    ...
    try {
        while (b != e) {
            #if DEBUG_OUTPUT >= 3
            ::std::ostringstream os;
            tag(os) << " Buffer size " << e - b << "\n";
            ::std::cerr << os.str();
            #endif
            if (!carry_.empty()) {
                #if DEBUG_OUTPUT >= 3
                ::std::ostringstream os;
                tag(os) << " Read message with carry. Carry size " << carry_.size() << "\n";
                ::std::cerr << os.str();
                #endif
                ...
                #if DEBUG_OUTPUT >= 3
                ::std::ostringstream os;
                tag(os) << " Read message. Buffer size " << e - b << "\n";
                ::std::cerr << os.str();
                #endif
                message m;
        ...
        if (b != e) {
            ::std::ostringstream os;
            tag(os) << " Add " << e - b << " bytes to the carry\n";
            ::std::cerr << os.str();
        }
        #endif
        ...
        #if DEBUG_OUTPUT >= 2
        ::std::ostringstream os;
        tag(os) << " Protocol read exception: " << e.what() << "\n";
        ::std::cerr << os.str();
        #endif
        ...
        functional::callback< bool > sent, bool one_way)
{
    #if DEBUG_OUTPUT >= 3
    ::std::ostringstream os;
    tag(os) << " Invocation #" << r_no << " has been sent\n";
    ::std::cerr << os.str();
    #endif
    pending_replies_type::accessor acc;
    if (pending_replies_.find(acc, r_no)) {
        ...
        functional::callback< bool > sent)
{
    ...
    #if DEBUG_OUTPUT >= 3
    ::std::ostringstream os;
    tag(os) << " Invoke request " << target.identity << "::" << op
            << " #" << r.number << "\n";
    ::std::cerr << os.str();
    #endif
...
connection_implementation::dispatch_reply(encoding::incoming_ptr buffer)
{
    ...
    try {
        ...
        #if DEBUG_OUTPUT >= 3
        ::std::ostringstream os;
        tag(os) << " Dispatch reply #" << rep.number << "\n";
        ::std::cerr << os.str();
        #endif
        ...
        if (pending_replies_.find(acc, rep.number)) {
            ...
            switch (rep.status) {
                case reply::success:{
                    #if DEBUG_OUTPUT >= 3
                    ::std::ostringstream os;
                    tag(os) << " Reply #" << rep.number << " status is success\n";
                    ::std::cerr << os.str();
                    #endif
                    if (p_rep.reply) {
                        ...
                        ::std::ostringstream os;
                        tag(os)
                                << " Reply encaps v" << ever.major << "." << ever.minor
                                << " size " << encaps.size() << "\n";
                        ::std::cerr << os.str();
                        #endif
                ...
                case reply::no_operation: {
                    ...
                    ::std::ostringstream os;
                    tag(os)
                            << " Reply #" << rep.number << " status is not found\n";
                    ::std::cerr << os.str();
                    #endif
                    if (p_rep.error) {
                        incoming::encaps_guard encaps{buffer->begin_encapsulation(b)};
                ...
                case reply::unknown_exception: {
                    #if DEBUG_OUTPUT >= 3
                    ::std::ostringstream os;
                    tag(os) << " Reply #" << rep.number << " status is an exception\n";
                    ::std::cerr << os.str();
                    #endif
                    if (p_rep.error) {
                        incoming::encaps_guard encaps{buffer->begin_encapsulation(b)};
            ...
            #if DEBUG_OUTPUT >= 4
            ::std::ostringstream os;
            tag(os) << " Pending replies: " << pending_replies_.size() << "\n";
            ::std::cerr << os.str();
            #endif
        } else {
            ...
            #if DEBUG_OUTPUT >= 4
            ::std::ostringstream os;
            tag(os) << " No waiting callback for reply\n";
            ::std::cerr << os.str();
            #endif
        }
    } catch (::std::exception const& e) {
        #if DEBUG_OUTPUT >= 2
        ::std::ostringstream os;
        tag(os) << " Exception when reading reply: " << e.what() << "\n";
        ::std::cerr << os.str();
        #endif
        connection_failure(::std::current_exception());
    } catch (...) {
        #if DEBUG_OUTPUT >= 2
        ::std::ostringstream os;
        tag(os) << " Exception when reading reply\n";
        ::std::cerr << os.str();
        #endif
        connection_failure(::std::current_exception());
    }
